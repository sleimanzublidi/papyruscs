...
namespace PapyrusCs.Output.OpenLayers
{
    public class OpenLayers
    {
        ...
        {
            this.WriteMapHtml(tileSize, outputPath, mapHtmlFile, settings, isUpdate);
            //this.AddPlayerIcons(outputPath, showPlayerIcons, world);
        }
...
var playersData = // # INJECT DATA HERE;";
        ...
        public void WriteMapHtml(int tileSize, string outputPath, string mapHtmlFile, Settings[] settings, bool isUpdate)
        {
            try
            {
                ...
                var mapHtmlContext = File.ReadAllText(Path.Combine(AppDomain.CurrentDomain.BaseDirectory, "map.html"));

                string getDimWithProfile(Settings setting)
                    => "dim" + setting.Dimension + (string.IsNullOrEmpty(setting.Profile) ? "" : $"_{setting.Profile}");
